...
interface DemandRepository extends JpaRepository<Demand, DemandId> {
    ...
    @Query("""
            select 
                new hexmatcher.demand.application.usecase.QueryNumberOfProposedCandidatesForDemandUseCase$CountProposedCandidateForDemandDto(d.demandId,count (c.candidateId))
            from Demand d
            ...
            """)
    CountProposedCandidateForDemandDto queryBy(@Param("demandId") DemandId demandId);
}
